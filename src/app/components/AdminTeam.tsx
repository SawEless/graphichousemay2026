...
const TeamMemberItem: React.FC<{ member: Member; index: number }> = ({ member, index }) => (
  <motion.div 
    ...
    className="bg-gray-900 rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-yellow-400/20 group font-sans"
  >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {teamMembers.map((member, i) => (
            <TeamMemberItem key={i} member={member} index={i} />
          ))}
        </div>
      </div>
    </section>
